...
async function main() {
  ...
  try {
    ...
    const dexRouter = await DexRouter.deploy(poolFactory.address, wethContractAddress);
    ...
    await weth.approve(dexRouter.address, utils.parseEther(
            wethInitialLiquidity
        ));

    await dai.approve(dexRouter.address, utils.parseEther(
            daiInitialLiquidity
        ));
    ...
    const [reserve0, reserve1] = await dexRouter.getTokenPairReserves(weth.address, dai.address);
    ...
    console.log(`Contract Pool Factory deployed to ${poolFactory.address} on ${network.name}`)
    console.log(`Contract Dex Router deployed to ${dexRouter.address} on ${network.name}`)
    console.log(
